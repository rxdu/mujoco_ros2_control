...
void MujocoRos2ControlPlugin::Configure(rclcpp::Node::SharedPtr &node,
                                        rclcpp::NodeOptions cm_node_option,
                                        mjModel *model, mjData *data) {
  control_ = std::make_unique<mujoco_ros2_control::MujocoRos2Control>(
      node, cm_node_option, model, data);
  control_->init();
  RCLCPP_INFO_STREAM(
      node->get_logger(),
